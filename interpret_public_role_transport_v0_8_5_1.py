#!/usr/bin/env python3
"""Deterministically interpret the sealed v0.8.5 -> v0.8.5.1 transition."""
...
import public_role_transport_canary_v0_8_5_1 as current
import typed_public_state_regression_v0_8_5 as prior


SCHEMA_VERSION = "ebrt-public-role-transport-interpretation-v0.8.5.1"

CLAIM_BOUNDARY = (
    "The known readiness task and all regression cases are contaminated engineering material.",
    "v0.8.5.1 changed the evidence records and one adapter-label line, so a role-only effect is not identified.",
...
def _project_public_roles(prompt: str) -> str:
    projected: list[str] = []
    for raw_line in prompt.splitlines():
        if raw_line.startswith("EVIDENCE_JSON "):
            try:
                value = json.loads(raw_line.removeprefix("EVIDENCE_JSON "))
            except json.JSONDecodeError as error:
                raise EBRTError("V0851_INTERPRET_EVIDENCE_JSON_INVALID") from error
            if not isinstance(value, dict) or set(value) != {
                "evidence_id",
                "role",
                "text",
            }:
                raise EBRTError("V0851_INTERPRET_ROLE_RECORD_INVALID")
            value.pop("role")
            raw_line = "EVIDENCE_JSON " + json.dumps(
                value,
                ensure_ascii=True,
                sort_keys=True,
                separators=(",", ":"),
...
def _prompt_audit() -> JsonObject:
    ...
    for label, old_prompt, new_prompt in pairs:
        projected = _project_public_roles(new_prompt)
        diff = list(
            difflib.unified_diff(
                old_prompt.splitlines(),
                projected.splitlines(),
                fromfile="v0.8.5",
                tofile="v0.8.5.1-minus-role",
                lineterm="",
            )
        )
        rows.append(
    ...
    non_role_diffs = {
        line
        for row in rows
        for line in row["non_role_unified_diff"]
        if line.startswith(("+You are", "-You are"))
    }
    return _seal(
        {
            "invocation_count": len(rows),
            "text_only_projection_exact_count": sum(
                row["text_only_projection_exact"] for row in rows
            ),
            "non_role_difference": "ADAPTER_LABEL_LINE",
            "non_role_label_lines": sorted(non_role_diffs),
...
def interpret(
    ...
    if set(old_runs) != set(new_runs):
        raise EBRTError("V0851_INTERPRET_MODEL_SET_MISMATCH")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        receipt = interpret(
            _load(args.prior_result, "V0851_INTERPRET_PRIOR_RESULT_INVALID"),
            _load(args.prior_lock, "V0851_INTERPRET_PRIOR_LOCK_INVALID"),
            _load(args.current_result, "V0851_INTERPRET_CURRENT_RESULT_INVALID"),
            _load(args.current_lock, "V0851_INTERPRET_CURRENT_LOCK_INVALID"),
        )
