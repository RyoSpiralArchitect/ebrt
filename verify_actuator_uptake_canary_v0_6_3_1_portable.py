#!/usr/bin/env python3
"""Pure-stdlib verifier for the EBRT v0.6.3.1 zero-call canary.
...
POLICY_RELATIVE = Path("policy_lock_actuator_uptake_canary_v0_6_3_1.json")
FIXTURE_RELATIVE = Path("fixtures/actuator_uptake_canary_v0_6_3_1.json")
GOLD_RELATIVE = Path("fixtures/actuator_uptake_canary_gold_v0_6_3_1.json")
ARTIFACT_RELATIVE = Path("artifacts/actuator_uptake_canary_v0_6_3_1_preflight")
...
GEOMETRY_SCHEMA = "ebrt-actuator-uptake-permutation-geometry-v0.6.3.1"
...
ARTIFACT_FILES = frozenset(
    {"projection_bundle.json", "controller_audit.json", "self_test.json", "manifest.json"}
)
...
def _validate_source_receipt(root: Path, receipt: Any, *, label: str) -> None:
    ...
    _require(type(receipt["bytes"]) is int and receipt["bytes"] >= 0, f"{label} bytes invalid")
...
def _candidate_id(selected: Sequence[str], salt: str) -> str:
    digest = hashlib.sha256(salt.encode("utf-8") + b":" + _canonical_bytes(list(selected))).hexdigest()
...
def _validate_fixture(fixture: Mapping[str, Any]) -> dict[str, Any]:
    _require(
        set(fixture)
        == {"schema_version", "status", "operator", "arms", "case", "protocol", "claim_boundary"},
        "fixture top-level keyset drifted",
    )
    _require(fixture.get("schema_version") == FIXTURE_SCHEMA, "fixture schema drifted")
    _require(fixture.get("status") == "LOCKED_NETWORK_ZERO_PREFLIGHT_INPUT", "fixture status drifted")
    ...
    for key in ("case_id", "checkpoint_id", "question"):
        _require(isinstance(case.get(key), str) and bool(case[key]), f"fixture {key} is invalid")
    _require(len(_unique_strings(case.get("answer_choices"), label="answer choices")) == 2, "answer choice count drifted")
    _require(len(_unique_strings(case.get("record_format_choices"), label="record formats")) == 1, "record format count drifted")
    ...
    _require(tuple(evidence_ids) == tuple(f"E{index}" for index in range(1, 8)), "evidence IDs drifted")
    _require(len(evidence_ids) == len(set(evidence_ids)), "evidence IDs contain duplicates")
    ...
    for key in ("neutral_order", "positive_control_order"):
        ...
        _require(set(order) == set(evidence_ids), f"protocol {key} is not a permutation")
    _require(protocol.get("matched_anti_rule") == "swap_preferred_and_opposed_path_blocks", "matched anti rule drifted")
    ...
    for key in ("execution_order_seed", "candidate_id_salt"):
        _require(isinstance(protocol.get(key), str) and bool(protocol[key]), f"protocol {key} is invalid")

    event = case.get("public_event_contract")
    _require(
        isinstance(event, Mapping)
        and set(event) == {"late_event_evidence_id", "invalidated_evidence_id", "stable_evidence_id"},
        "public event contract drifted",
    )
    _require(set(event.values()).issubset(evidence_ids), "public event references unknown evidence")
    _require(len(set(event.values())) == 3, "public event roles collide")

    candidates = case.get("candidate_closures")
    _require(isinstance(candidates, list) and len(candidates) == 4, "candidate count drifted")
    ...
    for index, candidate in enumerate(candidates):
        ...
        selected = _unique_strings(candidate.get("selected_evidence_ids"), label=f"candidate {index} evidence")
        _require(len(selected) == 4, f"candidate {index} cardinality drifted")
        _require(set(selected).issubset(evidence_ids), f"candidate {index} references unknown evidence")
    ...
    _require(len(candidate_ids) == len(set(candidate_ids)), "candidate IDs contain duplicates")
    _require(len(selected_sets) == len(set(selected_sets)), "candidate structures contain duplicates")
    ...
    _require(isinstance(blocks, Mapping) and set(blocks) == {"P0", "P1"}, "path blocks drifted")
    p0 = _unique_strings(blocks.get("P0"), label="P0")
    p1 = _unique_strings(blocks.get("P1"), label="P1")
    _require(len(p0) == len(p1) == 2 and not set(p0).intersection(p1), "path block geometry drifted")
    _require(set(p0) | set(p1) | set(event.values()) == set(evidence_ids), "evidence partition drifted")
    effects = controller.get("effect_by_evidence_id")
    _require(isinstance(effects, Mapping) and set(effects) == set(evidence_ids), "effect set drifted")
    ...
    for key in (
        ...
        _require(_finite(controller.get(key), label=key) > 0.0, f"{key} must be positive")
    return {
...
def _validate_gold(gold: Mapping[str, Any], fixture: Mapping[str, Any]) -> None:
    ...
    _require(gold.get("status") == "LOCKED_POST_CALL_GRADING_ONLY", "gold status drifted")
    case = fixture["case"]
    ...
    _require(gold.get("record_format") in case["record_format_choices"], "gold format invalid")
    roles = gold.get("closure_roles")
    _require(isinstance(roles, Mapping) and set(roles) == set(candidates), "gold role set drifted")
    _require(
        set(roles.values())
        == {
    ...
    valid = _unique_strings(gold.get("quality_valid_closure_ids"), label="quality-valid closures")
    _require(len(valid) == 2 and set(valid).issubset(candidates), "quality-valid closure set drifted")
    ...
    _require(preferred_selected in ids_by_selected, "preferred event-consistent closure is missing")
    _require(opposed_selected in ids_by_selected, "opposed event-consistent closure is missing")
    aligned_id = ids_by_selected[preferred_selected]
    alternative_id = ids_by_selected[opposed_selected]
    _require(target == aligned_id, "gold target is not bound to the preferred controller path")
    _require(set(valid) == {aligned_id, alternative_id}, "quality-valid set is not exactly the two event-consistent closures")
    _require(roles[aligned_id] == "ALIGNED_EVENT_CONSISTENT", "preferred closure role drifted")
    _require(roles[alternative_id] == "ALTERNATIVE_EVENT_CONSISTENT", "opposed closure role drifted")
    stale_ids = [closure_id for closure_id, role in roles.items() if role == "STALE_INVALIDATED_SUPPORT"]
...
def _controller_loss(
    controls: Sequence[float], effects: Sequence[float], *, decay: float, target: float, regularization: float
    ...
    for control, effect in zip(controls, effects, strict=True):
        ...
        states.append(state)
    loss = (state - target) ** 2 + regularization * math.fsum(value * value for value in controls)
    return loss, states
...
def _expected_controller(fixture: Mapping[str, Any]) -> dict[str, Any]:
    ...
    path_scores = {
        path_id: math.fsum(displacement_by_id[item] for item in spec["path_blocks"][path_id])
    ...
    return {
        ...
        "maximum_error": max(abs(left - right) for left, right in zip(gradients, finite_difference, strict=True)),
...
def _validate_controller(controller: Mapping[str, Any], fixture: Mapping[str, Any]) -> dict[str, Any]:
    ...
    _require(controller.get("schema_version") == CONTROLLER_SCHEMA, "controller schema drifted")
    _require(controller.get("case_id") == fixture["case"]["case_id"], "controller case drifted")
    ...
    _close(controller.get("baseline_loss"), expected["baseline_loss"], label="baseline loss")
    states = controller.get("baseline_states")
    _require(isinstance(states, list) and len(states) == 7, "baseline state count drifted")
    for index, value in enumerate(states):
        _close(value, expected["baseline_states"][index], label=f"baseline state {index}")
    for field, expected_key in (
        ...
        _require(isinstance(observed, Mapping) and set(observed) == set(expected_values), f"{field} keyset drifted")
    ...
    tolerance = float(fixture["case"]["local_controller"]["finite_difference_tolerance"])
    _close(controller.get("finite_difference_tolerance"), tolerance, label="finite-difference tolerance")
    _require(float(controller["maximum_finite_difference_error"]) <= tolerance, "finite-difference agreement failed")
    scores = controller.get("path_scores")
    _require(isinstance(scores, Mapping) and set(scores) == {"P0", "P1"}, "path score keyset drifted")
    for path_id, value in expected["path_scores"].items():
        _close(scores[path_id], value, label=f"path score {path_id}")
    _require(controller.get("preferred_path_id") == expected["preferred"], "preferred path drifted")
    _require(controller.get("opposed_path_id") == expected["opposed"], "opposed path drifted")
    _require(abs(float(scores["P0"]) - float(scores["P1"])) > FLOAT_TOLERANCE, "controller path preference is tied")
    return expected
...
def _positional_alignment(order: Sequence[str], displacement: Mapping[str, float]) -> float:
    return math.fsum(float(displacement[item]) * float(len(order) - index) for index, item in enumerate(order))
...
def _validate_geometry(
    geometry: Mapping[str, Any], fixture: Mapping[str, Any], controller: Mapping[str, Any]
) -> dict[str, tuple[str, ...]]:
    ...
    _require(geometry.get("schema_version") == GEOMETRY_SCHEMA, "geometry schema drifted")
    _require(geometry.get("case_id") == fixture["case"]["case_id"], "geometry case drifted")
    expected = _expected_orders(fixture, controller)
    observed_orders = geometry.get("orders")
    _require(isinstance(observed_orders, Mapping) and set(observed_orders) == set(ARMS), "geometry arm set drifted")
    for arm in ARMS:
    ...
    _require(isinstance(distances, Mapping) and set(distances) == {"D", "C"}, "geometry distance arms drifted")
    ...
    _require(computed_distances["D"] == computed_distances["C"], "D/C permutation geometry is not matched")
    ...
    _require(isinstance(alignments, Mapping) and set(alignments) == {"D", "C"}, "alignment keyset drifted")
    _close(alignments["D"], d_alignment, label="D positional alignment")
    _close(alignments["C"], c_alignment, label="C positional alignment")
    _close(geometry.get("d_minus_c_alignment"), d_alignment - c_alignment, label="D minus C alignment")
    event = fixture["case"]["public_event_contract"]
    checks = {
        "all_four_orders_distinct": len(set(expected.values())) == 4,
        "d_c_footrule_matched": computed_distances["D"]["spearman_footrule"] == computed_distances["C"]["spearman_footrule"],
        "d_c_kendall_matched": computed_distances["D"]["kendall"] == computed_distances["C"]["kendall"],
        "d_c_fixed_points_matched": computed_distances["D"]["fixed_points"] == computed_distances["C"]["fixed_points"],
        "d_c_anchor_positions_matched": all(
            expected["D"].index(str(event[key])) == expected["C"].index(str(event[key]))
            for key in ("late_event_evidence_id", "invalidated_evidence_id", "stable_evidence_id")
        ),
        "d_alignment_exceeds_c": d_alignment > c_alignment + FLOAT_TOLERANCE,
    }
    _require(geometry.get("checks") == checks and all(checks.values()), "geometry checks drifted")
    return expected


def _execution_order(seed: str) -> tuple[str, ...]:
    return tuple(sorted(ARMS, key=lambda arm: hashlib.sha256(f"{seed}|{arm}".encode()).hexdigest()))
...
def _normalized_payload(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    output["ordered_raw_evidence"] = sorted(output["ordered_raw_evidence"], key=lambda row: row["evidence_id"])
    return output


def _validate_payload(
    payload: Mapping[str, Any], *, fixture: Mapping[str, Any], expected_order: Sequence[str], label: str
) -> tuple[str, tuple[str, ...]]:
    ...
    case = fixture["case"]
    _require(raw_payload.get("schema_version") == PROVIDER_INPUT_SCHEMA, f"{label} schema drifted")
    _require(raw_payload.get("checkpoint_id") == case["checkpoint_id"], f"{label} checkpoint drifted")
    for key in ("question", "answer_choices", "record_format_choices", "candidate_closures"):
        _require(_canonical_bytes(raw_payload.get(key)) == _canonical_bytes(case[key]), f"{label} {key} drifted")
    _require(
    ...
    _require(isinstance(rows, list) and len(rows) == 7, f"{label} evidence count drifted")
    source_rows = {row["evidence_id"]: row for row in case["raw_evidence"]}
    observed_order: list[str] = []
    chunk_hashes: list[str] = []
    for index, row in enumerate(rows):
        _require(isinstance(row, Mapping) and set(row) == {"evidence_id", "text"}, f"{label} evidence row {index} drifted")
        evidence_id = row.get("evidence_id")
        _require(evidence_id in source_rows, f"{label} evidence row {index} is unknown")
        _require(_canonical_bytes(row) == _canonical_bytes(source_rows[evidence_id]), f"{label} evidence row {index} byte content drifted")
        observed_order.append(str(evidence_id))
        chunk_hashes.append(_sha256(_canonical_bytes(row)))
    _require(tuple(observed_order) == tuple(expected_order), f"{label} evidence order drifted")
    _require(len(observed_order) == len(set(observed_order)), f"{label} repeats evidence")
    return _fingerprint(raw_payload), tuple(sorted(chunk_hashes))


def _validate_projection(
    projection: Mapping[str, Any], *, fixture: Mapping[str, Any], controller: Mapping[str, Any]
) -> dict[str, Any]:
    ...
    _require(projection.get("schema_version") == PROJECTION_SCHEMA, "projection schema drifted")
    ...
    _require(projection.get("case_id") == fixture["case"]["case_id"], "projection case drifted")
    _require(projection.get("provider_payload_count") == 4, "projection payload count drifted")
    _require(projection.get("provider_calls_authorized") == 0, "projection authorizes provider calls")
    _require(projection.get("provider_calls_observed") == 0, "projection records provider calls")
    _require(
        projection.get("gold_used_for_projection") is False,
        "projection used post-call gold",
    )
    _require(
        projection.get("controller_audit_fingerprint_sha256") == controller["fingerprint_sha256"],
        "projection controller receipt drifted",
    )
    expected_orders = _validate_geometry(projection.get("permutation_geometry", {}), fixture, controller)
    seed = str(fixture["protocol"]["execution_order_seed"])
    expected_execution = _execution_order(seed)
    _require(tuple(projection.get("execution_order", ())) == expected_execution, "projection execution order drifted")

    payload_rows = projection.get("provider_payloads")
    treatment_rows = projection.get("public_treatment_key")
    _require(isinstance(payload_rows, list) and len(payload_rows) == 4, "projection must contain four payloads")
    _require(isinstance(treatment_rows, list) and len(treatment_rows) == 4, "projection must contain four treatment rows")
    payload_by_request: dict[str, Mapping[str, Any]] = {}
    for index, row in enumerate(payload_rows, start=1):
        _require(
            isinstance(row, Mapping)
            and set(row) == {"sequence_index", "blinded_request_id", "payload"},
            f"payload row {index} schema drifted",
        )
        _require(row.get("sequence_index") == index, f"payload row {index} sequence drifted")
    ...
    for index, row in enumerate(treatment_rows, start=1):
        _require(
            isinstance(row, Mapping)
            and set(row) == {"sequence_index", "blinded_request_id", "treatment_id"},
            f"treatment row {index} schema drifted",
        )
        _require(row.get("sequence_index") == index, f"treatment row {index} sequence drifted")
        arm = row.get("treatment_id")
        request_id = row.get("blinded_request_id")
        _require(arm == expected_execution[index - 1], f"treatment row {index} arm order drifted")
        _require(arm in ARMS and arm not in arm_to_payload, f"treatment row {index} arm invalid")
        _require(request_id in payload_by_request, f"treatment row {index} blind ID missing")
        _require(request_id == _blind_id(str(fixture["case"]["case_id"]), str(arm), seed), f"treatment row {index} blind ID drifted")
        arm_to_payload[str(arm)] = payload_by_request[str(request_id)]
    _require(set(arm_to_payload) == set(ARMS), "treatment mapping does not cover Z/C/D/X")
    _require(len(payload_by_request) == len(arm_to_payload), "treatment mapping does not cover every payload")
    ...
    for arm in ARMS:
        ...
        raw_fingerprint, chunks = _validate_payload(
            payload, fixture=fixture, expected_order=expected_orders[arm], label=f"payload {arm}"
        )
        raw_payload = _without_fingerprint(payload)
    ...
    _require(len(payload_fingerprints) == 4, "provider payload fingerprints are not four distinct values")
    ...
    _require(next(iter(chunk_multisets)) == tuple(sorted(_sha256(_canonical_bytes(row)) for row in fixture["case"]["raw_evidence"])), "provider evidence chunks differ from fixture")
    return {"orders": expected_orders, "payload_fingerprints": payload_fingerprints}


def _validate_self_test(
    self_test: Mapping[str, Any], *, controller: Mapping[str, Any], projection: Mapping[str, Any]
) -> None:
    ...
    _require(self_test.get("schema_version") == SELF_TEST_SCHEMA, "self-test schema drifted")
    _require(self_test.get("status") == SELF_TEST_STATUS, "self-test status drifted")
    gates = self_test.get("hard_gates")
    _require(isinstance(gates, list) and len(gates) == len(HARD_GATE_IDS), "self-test hard-gate count drifted")
    _require(
        gates == [{"gate_id": gate_id, "passed": True} for gate_id in HARD_GATE_IDS],
        "self-test hard gates drifted",
    )
    _require(self_test.get("controller_fingerprint_sha256") == controller["fingerprint_sha256"], "self-test controller receipt drifted")
    _require(self_test.get("projection_fingerprint_sha256") == projection["fingerprint_sha256"], "self-test projection receipt drifted")
    _require(self_test.get("classifier_combinations_exercised") == 256, "self-test classifier coverage drifted")
    _require(self_test.get("network_calls") == 0, "self-test network calls are nonzero")
    _require(self_test.get("provider_calls") == 0, "self-test provider calls are nonzero")
...
def _validate_policy(
    ...
    *,
    ...
    for key in ("python", "machine", "torch", "pydantic"):
        _require(isinstance(runtime.get(key), str) and bool(runtime[key]), f"runtime {key} is invalid")
    protocol_fixture = fixture["protocol"]
    for key in ("model", "reasoning_effort", "max_output_tokens", "timeout_seconds", "sdk_retries", "store"):
        _require(runtime.get(key) == protocol_fixture[key], f"runtime {key} drifted")
    _require(runtime.get("previous_response_id") is False, "runtime previous_response_id drifted")
    _require(runtime.get("provider_calls_authorized") == 0, "runtime authorizes provider calls")
    ...
    _require(protocol.get("case_id") == fixture["case"]["case_id"], "policy case drifted")
    _require(protocol.get("provider_payload_count") == 4, "policy payload count drifted")
    _require(protocol.get("execution_order") == projection["execution_order"], "policy execution order drifted")
    _require(protocol.get("orders") == projection["permutation_geometry"]["orders"], "policy permutation orders drifted")
    _require(protocol.get("hard_gate_ids") == list(HARD_GATE_IDS), "policy hard-gate order drifted")
    _require(protocol.get("primary_action_field") == "selected_closure_id", "policy primary action drifted")
    _require(protocol.get("positive_result") == "PROMOTE_TO_FRESH_REPLICATION", "policy positive result drifted")
    _require(protocol.get("direct_v0_6_4_promotion_allowed") is False, "policy directly promotes v0.6.4")
    _require(protocol.get("semantic_failures_are_endpoints") is True, "policy semantic endpoint rule drifted")
    _require(
    ...
    _require(isinstance(sources, Mapping) and set(sources) == SOURCE_RECEIPT_IDS, "policy source receipt set drifted")
    expected_source_paths = {
        "core": "actuator_uptake_canary_v0_6_3_1.py",
        "fixture": FIXTURE_RELATIVE.as_posix(),
        "post_call_gold": GOLD_RELATIVE.as_posix(),
        "protocol_note": "docs/RND_ACTUATOR_UPTAKE_CANARY_V0_6_3_1.md",
        "requirements": "requirements.txt",
    }
    for source_id, expected_path in expected_source_paths.items():
        receipt = sources[source_id]
        _require(isinstance(receipt, Mapping) and receipt.get("path") == expected_path, f"source {source_id} path drifted")
...
def _validate_manifest(
    ...
    *,
    ...
    _require(manifest.get("live_execution_authorized") is False, "manifest authorizes live execution")
    ...
    _require(manifest.get("policy_lock_fingerprint_sha256") == policy["fingerprint_sha256"], "manifest policy fingerprint drifted")
    _require(manifest.get("projection_fingerprint_sha256") == projection["fingerprint_sha256"], "manifest projection fingerprint drifted")
    _require(manifest.get("controller_fingerprint_sha256") == controller["fingerprint_sha256"], "manifest controller fingerprint drifted")
    _require(manifest.get("self_test_fingerprint_sha256") == self_test["fingerprint_sha256"], "manifest self-test fingerprint drifted")
    receipts = manifest.get("artifact_receipts")
    _require(isinstance(receipts, Mapping) and set(receipts) == RECEIPTED_ARTIFACT_FILES, "manifest artifact receipt set drifted")
    for filename in sorted(RECEIPTED_ARTIFACT_FILES):
        _validate_artifact_receipt(receipts[filename], raw=artifact_raw[filename], label=f"artifact {filename}")
...
def _validate_all_recorded_calls_zero(value: Any, *, label: str) -> int:
    ...
    if isinstance(value, Mapping):
        for key, child in value.items():
            if key in {"provider_calls", "provider_calls_authorized", "provider_calls_observed", "network_calls"}:
...
def _artifact_entry_names(root: Path) -> set[str]:
    ...
    _require(stat.S_ISDIR(directory_stat.st_mode) and not root.is_symlink(), "artifact root must be a real directory")
    entries = list(root.iterdir())
    _require({path.name for path in entries} == ARTIFACT_FILES, "artifact directory entry set drifted")
    for path in entries:
        file_stat = path.lstat()
        _require(stat.S_ISREG(file_stat.st_mode) and not path.is_symlink(), f"artifact entry must be a regular non-symlink file: {path.name}")
...
def verify(root: Path = ROOT, artifact_dir: Path | None = None) -> dict[str, Any]:
    ...
    _require(artifact_dir == root / ARTIFACT_RELATIVE, "non-canonical artifact directory")
    _artifact_entry_names(artifact_dir)

    policy_raw, policy = _strict_object(root / POLICY_RELATIVE, label="policy", canonical_pretty=True)
    ...
    for filename in sorted(ARTIFACT_FILES):
        raw, value = _strict_object(artifact_dir / filename, label=f"artifact {filename}", canonical_pretty=True)
    ...
    projection_audit = _validate_projection(projection, fixture=fixture, controller=controller)
    ...
    _require(sources["fixture"]["bytes"] == len(fixture_raw) and sources["fixture"]["sha256"] == _sha256(fixture_raw), "fixture source receipt drifted")
    _require(sources["post_call_gold"]["bytes"] == len(gold_raw) and sources["post_call_gold"]["sha256"] == _sha256(gold_raw), "gold source receipt drifted")
    call_fields = sum(
    ...
    return {
        "schema_version": "ebrt-actuator-uptake-portable-verification-v0.6.3.1",
        ...
        "provider_payload_fingerprints_verified": len(projection_audit["payload_fingerprints"]),
...
def self_test(root: Path = ROOT) -> dict[str, Any]:
    ...
    _, controller = _strict_object(root / ARTIFACT_RELATIVE / "controller_audit.json", label="controller probe", canonical_pretty=True)
    _, projection = _strict_object(root / ARTIFACT_RELATIVE / "projection_bundle.json", label="projection probe", canonical_pretty=True)
    ...
    _expect_rejected(
        lambda: _validate_projection(evidence_tamper, fixture=fixture, controller=controller),
        "immutable evidence chunk",
    )

    candidate_tamper = copy.deepcopy(projection)
    payload = candidate_tamper["provider_payloads"][0]["payload"]
    payload["candidate_closures"][0]["selected_evidence_ids"] = ["E1", "E2", "E6"]
    candidate_tamper["provider_payloads"][0]["payload"] = _seal(payload)
    candidate_tamper = _seal(candidate_tamper)
    _expect_rejected(
        lambda: _validate_projection(candidate_tamper, fixture=fixture, controller=controller),
        "candidate cardinality/catalog",
    )

    mapping_tamper = copy.deepcopy(projection)
    mapping_tamper["public_treatment_key"][0]["treatment_id"] = "Z"
    mapping_tamper = _seal(mapping_tamper)
    _expect_rejected(
        lambda: _validate_projection(mapping_tamper, fixture=fixture, controller=controller),
