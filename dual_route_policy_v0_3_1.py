#!/usr/bin/env python3
"""Factorized dual-route policy for the frozen EBRT mechanism.
...
* execution replay floor: where physical suffix recomputation starts; and
* trajectory anchor floor: where the loss begins constraining state drift.

The online leverage feature is a prefix-only feasible centered or
projected-forward one-sided finite difference. It never reads an observation or
state after the event source. In ``matched`` mode every arm pays the same probe
cost; in ``native`` mode only L2, D2, and G2 run the probe because only those
policies consume its ranking.
...
SCHEMA_VERSION = "ebrt-dual-route-policy-v0.3.1"
...
EXPECTED_INSTRUMENTATION_SHA256 = (
    "841325d2ba1f126c61750a9445710084a07530e5940b2246b36846a3c0cc3b88"
)
...
def _trajectory_anchor_loss(
    states: torch.Tensor,
    original_states: torch.Tensor,
    floor: int,
) -> torch.Tensor:
    """Return the loss-horizon term independently of physical replay."""

    if states.shape != original_states.shape:
        raise ValueError("trajectory anchor states must have the same shape")
    if floor < 0 or floor >= int(states.shape[0]):
        raise ValueError(f"trajectory anchor floor is out of range: {floor}")
    return (states[floor:] - original_states[floor:]).square().mean()
...
class RoutePlan:
    """One event's separated semantic, control, execution, and loss decisions."""
...
class DualRoutePolicyReasoner(observer.InstrumentedEventDrivenBackwardReasoner):
    ...
    def __init__(
        ...
        *,
        ...
        execution_replay_policy: str = "minimum_eligible_step",
        trajectory_anchor_policy: str = "minimum_eligible_step",
        route_seed: int = 0,
        ...
        if execution_replay_policy not in FLOOR_POLICIES:
            raise ValueError(
                "execution_replay_policy must be one of "
                f"{FLOOR_POLICIES}, got {execution_replay_policy!r}"
            )
        if trajectory_anchor_policy not in FLOOR_POLICIES:
            raise ValueError(
                "trajectory_anchor_policy must be one of "
                f"{FLOOR_POLICIES}, got {trajectory_anchor_policy!r}"
            )
        if route_capacity != 2:
            raise ValueError("v0.3.1 matched policy requires route_capacity=2")
        ...
        self.execution_replay_policy = execution_replay_policy
        self.trajectory_anchor_policy = trajectory_anchor_policy
    ...
    def _prefix_leverage_probe(
        ...
        with torch.no_grad():
            for candidate_step in candidates:
                ...
                mask = torch.zeros_like(controls)
                mask[candidate_step] = 1.0
                plus_delta = torch.zeros_like(controls)
                minus_delta = torch.zeros_like(controls)
                plus_delta[candidate_step] = self.leverage_epsilon * direction
                minus_delta[candidate_step] = -self.leverage_epsilon * direction
                requested_plus = controls[candidate_step] + plus_delta[candidate_step]
                requested_minus = controls[candidate_step] + minus_delta[candidate_step]
                requested_plus_norm = float(
                    torch.linalg.vector_norm(requested_plus).item()
                )
                requested_minus_norm = float(
                    torch.linalg.vector_norm(requested_minus).item()
                )
                feasibility_tolerance = 1e-6
                plus_requested_feasible = (
                    requested_plus_norm
                    <= float(self.config.max_control_norm) + feasibility_tolerance
                )
                minus_requested_feasible = (
                    requested_minus_norm
                    <= float(self.config.max_control_norm) + feasibility_tolerance
                )
                self._project_controls_(controls, plus_delta, mask)
                self._project_controls_(controls, minus_delta, mask)
                plus_all_controls = controls + mask * plus_delta
                minus_all_controls = controls + mask * minus_delta
                plus_controls = plus_all_controls[candidate_step : source_step + 1]
                minus_controls = minus_all_controls[candidate_step : source_step + 1]
                ...
                if boundary_limited:
                    finite_difference_scheme = "projected_forward_one_sided"
                    source_derivative = (
                        plus_states[-1] - prefix_states[source_step]
                    ) / self.leverage_epsilon
                else:
                ...
                plus_control_norm = float(
                    torch.linalg.vector_norm(plus_all_controls[candidate_step]).item()
                )
                minus_control_norm = float(
                    torch.linalg.vector_norm(minus_all_controls[candidate_step]).item()
                )
                probe_control_norm_max = max(plus_control_norm, minus_control_norm)
                if probe_control_norm_max > float(self.config.max_control_norm) + 1e-5:
                    raise AssertionError("online probe exceeded the frozen control bound")
                ...
                rows.append(
                    {
                        ...
                        "requested_epsilon": self.leverage_epsilon,
                        ...
                        "control_norm_before": float(
                            torch.linalg.vector_norm(controls[candidate_step]).item()
                        ),
                        ...
                        "probe_control_norm_max": probe_control_norm_max,
        ...
        scheme_counts = {
            scheme: sum(
                int(row["finite_difference_scheme"] == scheme) for row in rows
            )
            for scheme in ("centered", "projected_forward_one_sided")
        }
        return {
            "source_step": int(source_step),
            "performed": True,
            "method": (
                "prefix_only_feasible_centered_or_projected_forward_one_sided_"
                "topic_aligned_control"
            ),
            "metric": "target_aligned_event_source_belief_projection_derivative",
            "epsilon": self.leverage_epsilon,
            "max_control_norm": float(self.config.max_control_norm),
            ...
            "matched_probe_charge_contract": (
                "Both projected endpoints are executed for every candidate so probe "
                "work remains capacity-matched; boundary leverage uses only the "
                "forward target-oriented endpoint against the feasible base."
            ),
    ...
    def _build_route_plan(
        ...
        *,
        ...
        floor_by_policy = {
            "minimum_eligible_step": min(candidates),
            "minimum_selected_control_step": min(controls),
        }
        execution_replay_floor = floor_by_policy[self.execution_replay_policy]
        trajectory_anchor_floor = floor_by_policy[self.trajectory_anchor_policy]
        return RoutePlan(
            arm=self.arm,
            source_step=int(source_step),
            objective_anchor_steps=tuple(sorted(objective)),
            control_steps=tuple(sorted(controls)),
            execution_replay_floor=execution_replay_floor,
            trajectory_anchor_floor=trajectory_anchor_floor,
            ...
            event_ordinal=event_ordinal,
            execution_replay_policy=self.execution_replay_policy,
            trajectory_anchor_policy=self.trajectory_anchor_policy,
        )
    ...
    def _detect_event(
        ...
        candidate_record["execution_replay_floor"] = plan.execution_replay_floor
        candidate_record["trajectory_anchor_floor"] = plan.trajectory_anchor_floor
    ...
    def _revision_energy(
        ...
        trajectory_anchor = _trajectory_anchor_loss(
            states,
            original_states,
            plan.trajectory_anchor_floor,
        )
            ...
            self._deep_current.append(
                {
                    ...
                    "execution_replay_floor": plan.execution_replay_floor,
                    "trajectory_anchor_floor": plan.trajectory_anchor_floor,
                }
            )
        return total, terms
    ...
    def _revise_prefix(
        ...
        """Run the optimizer with independently selected execution and loss floors."""
        ...
        execution_replay_floor = int(plan.execution_replay_floor)
        trajectory_anchor_floor = int(plan.trajectory_anchor_floor)
        if execution_replay_floor > min(plan.control_steps):
            raise AssertionError("execution replay cannot skip a selected control")
        ...
        def replay_suffix(candidate_controls: torch.Tensor) -> torch.Tensor:
            if execution_replay_floor == 0:
                return self.generator.rollout(encoded, candidate_controls)
            unchanged_prefix = original_states[:execution_replay_floor]
            replayed_suffix = self.generator.rollout(
                encoded[execution_replay_floor:],
                candidate_controls[execution_replay_floor:],
        ...
        try:
            ...
            with torch.no_grad():
                ...
                affected_states_before = [
                    ...
                    for row in original_states[execution_replay_floor:].tolist()
                ]
                affected_states_after = [
                    [float(value) for value in row]
                    for row in final_states[execution_replay_floor:].tolist()
                ]
        finally:
            self._active_plan = None
        ...
        record = frozen.RevisionRecord(
            ...
            earliest_replay_step=execution_replay_floor,
            replayed_state_steps=(self.config.revision_steps + 2)
            * (event.source_step - execution_replay_floor + 1),
        ...
        self._revision_policy_records.append(
            {
                ...
                "control_steps": list(plan.control_steps),
                "execution_replay_floor": execution_replay_floor,
                "execution_replay_span_steps": (
                    event.source_step - execution_replay_floor + 1
                ),
                "trajectory_anchor_floor": trajectory_anchor_floor,
                "trajectory_anchor_span_steps": (
                    event.source_step - trajectory_anchor_floor + 1
                ),
    ...
    def run(self, observations: Sequence[Any]) -> Any:
        ...
        self.last_trace["dual_route_policy"] = {
            ...
            "probe_contract": (
                "Feasible centered or projected-forward one-sided finite differences "
                "stop at the event source and never read a future observation, "
                "state, or terminal outcome."
            ),
            "matched_contract": (
                "All matched-mode arms pay the same prefix probe, two-slot capacity, "
                "configured execution floor policy, trajectory-anchor policy, "
                "optimizer budget, and norm caps."
            ),
            "replay_contract": (
                "Physical suffix recomputation uses execution_replay_floor only; "
                "trajectory regularization uses trajectory_anchor_floor only. "
                "Probe charging does not select either floor."
            ),
...
def run_self_tests() -> dict[str, Any]:
    ...
    for arm in ARMS:
        ...
        if plan.execution_replay_floor != 0:
            raise AssertionError(f"{arm} did not use the common execution floor")
        if plan.trajectory_anchor_floor != 0:
            raise AssertionError(f"{arm} did not use the common loss floor")
        if plan.execution_replay_policy != "minimum_eligible_step":
            raise AssertionError(f"{arm} recorded the wrong execution policy")
        if plan.trajectory_anchor_policy != "minimum_eligible_step":
            raise AssertionError(f"{arm} recorded the wrong trajectory policy")
    ...
    for arm, expected_probe in native_probe_expectation.items():
        ...
        if native_plan.execution_replay_floor != min(native_plan.control_steps):
            raise AssertionError(f"cost lane did not use {arm}'s selected floor")
        if native_plan.trajectory_anchor_floor != min(native_plan.candidate_steps):
            raise AssertionError(f"cost lane changed {arm}'s loss horizon")
        if not engine.accounting["generator_accounting_ok"]:
            raise AssertionError(f"native {arm} generator accounting failed")
        native[arm] = engine
    if (
        native["L2"].route_plans[0].execution_replay_floor
        == matched["L2"][0].route_plans[0].execution_replay_floor
    ):
        raise AssertionError("native L2 fixture did not exercise natural replay cost")

    matched_l2_engine, matched_l2_result = matched["L2"]
    cost_l2_engine = DualRoutePolicyReasoner(
        config,
        arm="L2",
        case_id="multi-candidate-fixture",
        probe_mode="matched",
    ...
    cost_l2_result = cost_l2_engine.run(observations)
    matched_l2_plan = matched_l2_engine.route_plans[0]
    cost_l2_plan = cost_l2_engine.route_plans[0]
    if matched_l2_plan.control_steps != cost_l2_plan.control_steps:
        raise AssertionError("cost lane changed the selected control route")
    if matched_l2_plan.objective_anchor_steps != cost_l2_plan.objective_anchor_steps:
        raise AssertionError("cost lane changed the objective anchor")
    if matched_l2_plan.trajectory_anchor_floor != cost_l2_plan.trajectory_anchor_floor:
        raise AssertionError("cost lane changed the trajectory loss horizon")
    if not torch.equal(matched_l2_result.controls, cost_l2_result.controls):
        raise AssertionError("execution-only floor change altered committed controls")
    if not torch.equal(matched_l2_result.final_states, cost_l2_result.final_states):
        raise AssertionError("execution-only floor change altered final states")
    if matched_l2_result.decoded != cost_l2_result.decoded:
        raise AssertionError("execution-only floor change altered decoded output")
    if _session_fingerprint(matched_l2_result) != _session_fingerprint(cost_l2_result):
        raise AssertionError("execution-only floor change altered outcome fingerprint")
    if not (
        cost_l2_engine.accounting["optimizer_replay_steps"]
        < matched_l2_engine.accounting["optimizer_replay_steps"]
    ):
        raise AssertionError("execution-only floor change did not reduce replay work")

    factorial_l2_engine = DualRoutePolicyReasoner(
        config,
        arm="L2",
        case_id="multi-candidate-fixture",
        probe_mode="matched",
    ...
    factorial_l2_engine.run(observations)
    factorial_l2_plan = factorial_l2_engine.route_plans[0]
    if factorial_l2_plan.control_steps != matched_l2_plan.control_steps:
        raise AssertionError("trajectory factorial changed the selected route")
    if (
        factorial_l2_plan.execution_replay_floor
        != matched_l2_plan.execution_replay_floor
    ):
        raise AssertionError("trajectory factorial changed physical replay")
    if (
        factorial_l2_plan.trajectory_anchor_floor
        == matched_l2_plan.trajectory_anchor_floor
    ):
        raise AssertionError("trajectory factorial did not change the loss horizon")
    for field in (
        "actual_generator_step_calls",
        "actual_backward_calls",
        "optimizer_replay_steps",
    ):
        if factorial_l2_engine.accounting[field] != matched_l2_engine.accounting[field]:
            raise AssertionError(f"trajectory factorial changed accounting: {field}")

    anchor_states = torch.tensor([[0.0], [1.0], [2.0]])
    anchor_original = torch.zeros_like(anchor_states)
    if not (
        _trajectory_anchor_loss(anchor_states, anchor_original, 0)
        != _trajectory_anchor_loss(anchor_states, anchor_original, 1)
    ):
        raise AssertionError("trajectory anchor floor did not change loss support")

    first_engine, first_result = matched["D2"]
    boundary_engine = DualRoutePolicyReasoner(
        config,
        arm="D2",
        case_id="boundary-probe-fixture",
        probe_mode="matched",
        gold_steps_by_source=gold,
    )
    boundary_observations = observations[:5]
    boundary_codec = frozen.StructuredSemanticCodec(config)
    boundary_codec.prepare_topics(boundary_observations)
    boundary_encoded = boundary_codec.encode_many(boundary_observations)
    boundary_controls = torch.zeros(
        len(boundary_observations),
        config.control_dim,
        device=boundary_encoded.device,
        dtype=boundary_encoded.dtype,
    )
    boundary_topic = boundary_codec.topic_vector(boundary_observations[-1].topic)
    boundary_direction = (
        boundary_engine.generator.control_basis.transpose(0, 1) @ boundary_topic
    )
    boundary_direction = boundary_direction / torch.linalg.vector_norm(
        boundary_direction
    )
    boundary_controls[0] = config.max_control_norm * boundary_direction
    boundary_states = boundary_engine.generator.rollout(
        boundary_encoded,
        boundary_controls,
    ).detach()
    boundary_engine._committed_controls = boundary_controls.detach().clone()
    boundary_probe = boundary_engine._prefix_leverage_probe(
        observations,
        4,
        boundary_states,
        first_result.events[0],
        first_engine._event_candidates[0],
    )
    boundary_rows = [
        row for row in boundary_probe["rows"] if int(row["candidate_step"]) == 0
    ]
    if len(boundary_rows) != 1 or not boundary_rows[0]["boundary_limited"]:
        raise AssertionError("online probe did not detect the control boundary")
    if boundary_rows[0]["finite_difference_scheme"] != "projected_forward_one_sided":
        raise AssertionError("online boundary probe used an unsafe finite difference")
    if (
        float(boundary_rows[0]["probe_control_norm_max"])
        > float(config.max_control_norm) + 1e-5
    ):
        raise AssertionError("online boundary probe exceeded the control ball")
    ...
    return {
        ...
        "checks": [
            ...
            "objective, control, execution replay, and trajectory loss roles are separate",
            "matched arms share exact probe, replay, and total generator work",
            "native mode probes only L2, D2, and G2",
            "probe charging does not choose either floor",
            "execution-only floor shortening preserves exact outcomes and lowers replay work",
            "trajectory-only floor changes preserve routes and generator accounting",
            "online leverage probes stay feasible at the control boundary",
        ...
        "cost_lane_optimizer_replay_steps": {
            "common_execution_floor": matched_l2_engine.accounting[
                "optimizer_replay_steps"
            ],
            "selected_execution_floor": cost_l2_engine.accounting[
                "optimizer_replay_steps"
            ],
        },
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="EBRT v0.3.1 factorized dual-route policy"
    )
    parser.add_argument(
        "--self-test",
        action="store_true",
